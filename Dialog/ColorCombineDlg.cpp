...
BOOL CColorCombineDlg::OnInitDialog()
{
	...
	CMyIPTool2Doc* pDoc = NULL;
	POSITION pos = pApp->m_pImageDocTemplate->GetFirstDocPosition();

	while (pos != NULL)
	{
		pDoc = (CMyIPTool2Doc*)pApp->m_pImageDocTemplate->GetNextDoc(pos);
		if (pDoc->m_Mat.depth() != CV_8U && pDoc->m_Mat.depth() != CV_16U)	continue;
	...
	if (nIndex == 0)
	{
		AfxMessageBox(_T("그레이 스케일 영상이 없습니다."));
...
void CColorCombineDlg::OnBnClickedOk()
{
	...
	m_pDoc3 = (CMyIPTool2Doc*)m_comboImage3.GetItemDataPtr(m_comboImage3.GetCurSel());
	CDialogEx::OnOK();
}
